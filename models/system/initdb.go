...
func InitDb() error {
	...
	if config2.DatabaseConfig.Dbtype == "sqltite3" {
		fmt.Println("sqltite3无需初始化！")
		return nil
	}

	sqlcont, err := Ioutil(filePath)
	if err != nil {
		fmt.Println("数据库基础数据初始化脚本读取失败！ 原因：", err.Error())
		return err
	}

	sqlList := strings.Split(sqlcont, ";")
