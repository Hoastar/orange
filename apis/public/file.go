...
func UploadFile(c *gin.Context) {
	...
	if fileType != "images" && fileType != "files" {
		app.Error(c, -1, fmt.Errorf("上传接口仅支持图片上传与文件上传"), "")
		return
	}
	...
	if viper.GetBool("setting.domain.getHost") {
	...
	guid := strings.ReplaceAll(uuid.New().String(), "_", "")
	...
	case "1": // 单图
		...
		// 上传文件至指定目录
		singleFile := saveFilePath + guid + "_" + files.Filename
