...
type SysRole struct {
	RoleId	int `json:"roleId" gorm:"primary_key;AUTO_INCREMENT"`	// 角色代码
...
func (role *SysRole) GetRoleMenuId() ([]int, error) {
	...
	if err := orm.Eloquent.Table("sys_role_menu").Select("sys_role_menu.menu_id").
		Joins("LEFT JION sys_menu on sys_menu.menu_id").
		Where("role_id = ?", role.RoleId).
		Where("sys_role_menu.menu_id not in(select sys_menu.parent_id from sys_role_menu LEFT JOIN sys_menu on sys_menu.menu_id=sys_role_menu.menu_id where role_id =? )", role.RoleId).
...
func (e *SysRole) BatchDelete(id []int) (Result bool, err error) {
	if err = orm.Eloquent.Table("sys_role").Where("role in (?)", id).Delete(&SysRole{}).Error;
	err != nil {
		return
	}
...
func (role *SysRole) GetRoleDeptId() ([]int, error) {
	...
	if err := orm.Eloquent.Table("sys_role_dept").Select("sys_role_dept.dept_id").
		Joins("LEFT JOIN sys_dept on sys_dept.dept_id=sys_role_dept.dept_id").Where("role_id = ? ", role.RoleId).Where("sys_role_dept.dept_id not in(select sys_dept.parent_id from sys_role_dept LEFT JOIN sys_dept on sys_dept.dept_id=sys_role_dept.dept_id where role_id =? )", role.RoleId).
